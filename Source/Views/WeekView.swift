...
import UIKit
import SwiftDate
import TimelineCollectionView

@IBDesignable final class WeekView: UIView {
    ...
    private typealias CellType = WeekViewDayCell
    private lazy var timelineCollectionView: TimelineCollectionView<DataType, CellType> = {
        let config = TimelineCollectionViewConfig(visibleCells: visibleDays, scrollDirection: .horizontal)
        ...
        let tcv = TimelineCollectionView<DataType, CellType>(frame: rect, config: config)
    ...
    public weak var dataSource: WeekViewDataSource? {
        didSet {
    ...
    public weak var styler: WeekViewStyler? {
        didSet {
            if oldValue != nil {
...
extension WeekView: WeekViewStyler {
    ...
    func weekViewStylerEventView(_ weekView: WeekView, eventContainer: CGRect, event: WeekViewEvent) -> UIView {
        let weekViewEventView: WeekViewEventView = .fromNib()
...
// MARK: - TimelineCollectionViewDataSource

extension WeekView: TimelineCollectionViewDataSource {
    func timelineCollectionView<T, U>(_ timelineCollectionView: TimelineCollectionView<T, U>, dataFor index: Int, asyncClosure: @escaping (T?) -> Void) -> T? where U : UICollectionViewCell {
...
// MARK: - TimelineCollectionViewCellDelegate

extension WeekView: TimelineCollectionViewCellDelegate {
